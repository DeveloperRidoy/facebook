  ...
  return (
    <div className="dark:bg-dark p-2 rounded-lg">
      ...
      <div className="grid grid-cols-3 items-center gap-4 justify-between mt-3 max-h-52 overflow-auto">
        {ownProfile
          ? state.user?.friends.map((friend) => <Friend key={friend._id} friend={friend} state={state}/>)
          : user?.friends?.map((friend) => <Friend key={friend._id} friend={friend} state={state}/>)}
      </div>
    </div>
  );
};
...
const Friend = ({ friend, state }) => {
  ...
  const otherFriend = friend.requester._id === state.user?._id ? friend.recepient: friend.requester
  return (
    <Link href={otherFriend?.slug || "/"}>
